...
- **Prompt Node Input** — Fixed cursor jumping when typing in prompt nodes by using local state ([#37](https://github.com/shrimbly/node-banana/pull/37))
  - Text input now remains stable during typing
  - Changes persist to store only on blur for better performance

- **Indexed Handle Connections** — Fixed connection handling for indexed handles like `text-0`, `image-0` ([#37](https://github.com/shrimbly/node-banana/pull/37))
...
- **Video Node Validation** — Added validation for generateVideo nodes requiring text input ([#37](https://github.com/shrimbly/node-banana/pull/37))
...
- **Community Workflows API** — Hardcoded API URL to prevent errors from placeholder in `.env.example` ([#37](https://github.com/shrimbly/node-banana/pull/37))
