...
- Canvas automatically swaps full-res images for JPEG thumbnails when nodes are zoomed out, improving performance on large workflows ([#99](https://github.com/shrimbly/node-banana/pull/99))
...
- Split Grid node Split button and image preview now update immediately when an edge is connected, without needing to execute first ([#99](https://github.com/shrimbly/node-banana/pull/99))
...
- Thumbnail cache key collisions that could cause wrong images to appear on nodes ([#99](https://github.com/shrimbly/node-banana/pull/99))
