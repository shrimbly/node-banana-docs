...
- Keyboard shortcuts dialog (press `?`) now lists Shift+T (Add Audio node) and Shift+R (Add Array node) ([#85](https://github.com/shrimbly/node-banana/pull/85))
...
- Model schema now resolves `anyOf`/`oneOf`/`allOf` wrappers, fixing connectable handle detection for nullable image/text inputs (e.g., fal Kling v2.6 image-to-video) ([#85](https://github.com/shrimbly/node-banana/pull/85))
- Workflow JSON files no longer include the absolute `directoryPath` field, preventing local filesystem paths from being exposed when sharing workflow files ([#85](https://github.com/shrimbly/node-banana/pull/85))
