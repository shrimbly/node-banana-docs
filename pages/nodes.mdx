...
| [Generate Video](#generate-video) | AI video generation | Image, Audio, Text | Video |
...
| [Array](#array) | Split text into items and route to nodes | Text | Text (per item) |
...
5. Connect the audio output to a Video Stitch node to add audio to videos, or to a Generate Video node for audio-driven generation
...
- **Audio** (optional) — Audio input for audio-driven video generation (requires a model that supports audio-to-video)
...
The Array node splits a block of text into a list of items and routes each item to downstream nodes. Use it to run a batch of prompts or inputs through generation nodes in a single workflow.

### Inputs
- **Text** — The source text to split into items

### Outputs
- **Text** — One item per connected edge (each downstream node receives a different item)

### Settings

| Setting | Description |
|---------|-------------|
| **Split mode** | How to divide the text: Lines, Delimiter, Regex, or JSON |
| **Trim items** | Strip leading/trailing whitespace from each item |
| **Remove empty** | Discard blank items after splitting |
| **Batch** | When enabled, sends all items sequentially to a single downstream node instead of routing each item to a separate node |
| **Auto-route** | Automatically create and connect one downstream node per item |

### Batch Mode

Enable **Batch** to run all array items through one generate node in sequence. The node executes once per item, and all results accumulate in the output gallery.

Without batch mode (default), each output edge carries a different item — connect multiple downstream nodes to process items in parallel.

### Usage
1. Add an Array node to the canvas
2. Connect a Prompt node or LLM output to provide the source text
3. Choose a split mode and configure delimiter or regex as needed
4. Either enable **Batch** and connect a single downstream generate node, or use **Auto-route** to create one node per item

<Callout type="tip">
  Use **Batch** when you want a single generate node to process every item and collect all results. Use the default multi-edge routing when you want items processed in parallel across separate nodes.
</Callout>
