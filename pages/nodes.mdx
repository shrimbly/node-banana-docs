...
| [Generate Audio](#generate-audio) | AI audio generation | Text | Audio |
...
| [Output Gallery](#output-gallery) | View image/video collections | Image (multiple) | — |
...
| **Fallback Model** | A backup model used automatically if the primary model fails |

### Fallback Model

All generation nodes have a **Fallback** tab in their settings panel. Configure a secondary provider and model here; if the primary generation request fails (API error, rate limit, etc.), the node automatically retries with the fallback model. A visual indicator on the node shows when the fallback was used.

### Download

A download button on the node header saves the currently displayed image to your local machine.
...
Kie.ai supports a broad range of video generation models:

| Model | Type | Notes |
|-------|------|-------|
| Kling 3.0 | Text → Video, Image → Video | 5–10 second clips, pro mode |
| Kling 3.0 Motion Control | Image → Video | Character orientation and background controls |
| Kling 2.6 | Text → Video, Image → Video | Previous generation |
| Wan 2.7 (text-to-video) | Text → Video | 1080p, configurable duration and ratio |
| Wan 2.7 (image-to-video) | Image + Text → Video | 1080p output |
| Seedance 2.0 | Text → Video, Image → Video | 720p, audio generation support |
| Seedance 2.0 Fast | Text → Video, Image → Video | Faster variant of Seedance 2.0 |

### Video Generation Features

- **Extended timeout** — 10-minute timeout for longer video processing
- **Client-side polling** — Kie.ai tasks return immediately; the client polls for completion so the UI stays responsive
- **Video playback** — In-node video player with controls
- **Download button** — Save the generated video directly from the node header
- **Format detection** — Automatic handling of various video formats
- **Fallback model** — Configure a backup model in the Fallback settings tab
...
The Generate Audio node creates audio using AI models from providers that support audio generation.

### Inputs
- **Text** — The prompt describing the audio to generate
- **Dynamic inputs** — Additional inputs based on selected model's schema
...
| Setting | Description |
|---------|-------------|
| **Provider** | Choose from providers with audio capabilities |
| **Model** | Select from available audio models |
...
- **Download button** — Save the generated audio directly from the node header
- **Fallback model** — Configure a backup model in the Fallback settings tab
- **Generation history** — Browse previous generations with the carousel

### Usage
1. Add a Generate Audio node
2. Select a provider and audio-capable model
3. Connect a Prompt node describing the audio
4. Configure model-specific parameters
5. Run the workflow
6. Connect the audio output to a Video Stitch node to add it to a video
...
- **Download button** — Save the generated 3D model directly from the node header
- **Fallback model** — Configure a backup model in the Fallback settings tab
...
### Inputs
- **Image** (multiple) — Connect multiple image or video sources to view them in a grid
...
- **Video support**: Gallery displays videos as well as images
- **Extract button**: Creates a batch of Image Input nodes on the canvas from all items currently shown in the gallery — useful for feeding gallery results back into a workflow
- **Real-time display**: Shows media from connected nodes immediately, not just after execution
- **Automatic collection**: Gathers media from all connected image-producing nodes (Image Input, Generate Image, Annotation, etc.)

### Usage
1. Add an Output Gallery node
2. Connect multiple image or video sources to its input handle
3. The gallery automatically displays all connected media as thumbnails
4. Click any thumbnail to view full-size
5. Use keyboard shortcuts or navigation buttons to browse
6. Click the download button in lightbox to save individual items
7. Click **Extract** to create Image Input nodes from all gallery items
...
<Callout type="tip">
  **Extract to workflow**: Use the Extract button to take gallery results and batch-create Image Input nodes, making it easy to feed multiple generated images into further processing steps.
</Callout>
