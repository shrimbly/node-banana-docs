...
| [Video Stitch](#video-stitch) | Concatenate videos | Video (multiple), Audio | Video |
...
The Video Stitch node concatenates multiple video clips into a single output video. Perfect for creating compilations, montages, or multi-clip sequences.
...
| Setting | Description |
|---------|-------------|
| **Output Duration** | Total duration of the stitched video (automatically calculated from clips) |

### Usage

1. Add a Video Stitch node to the canvas
2. Connect at least 2 video sources to the video input handles
3. (Optional) Connect an Audio Input node to add background music
4. Drag thumbnails in the filmstrip to reorder clips
5. Run the workflow to generate the stitched video
6. Preview the result in the node or connect to an Output node

<Callout type="info">
  All video clips must have consistent dimensions. If clips have different resolutions, the Video Stitch node will display an error. Use video generation settings or preprocessing to ensure matching dimensions.
</Callout>

<Callout type="tip">
  Audio from the Audio Input is automatically adjusted to match the final video length. If the audio is longer, it's trimmed. If shorter, it plays until completion and the remaining video is silent.
</Callout>
...
For example, `easeInQuad` starts slow (duplicating frames for slow-motion) and accelerates (skipping frames for time-lapse).
...
- **Quality preservation**: No re-encoding quality loss
...
| Option | Grid Size |
|--------|-----------|
| 1×5 | 5 cells (useful for A+ content strips) |
| 2×2 | 4 cells |
| 2×3 | 6 cells |
| 2×4 | 8 cells |
| 3×3 | 9 cells |
