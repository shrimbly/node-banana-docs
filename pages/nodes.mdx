...
Four Nano Banana models are available via the Gemini provider:
...
| **Nano Banana 2 Lite** | 1K only | 10 standard | — | — |
| **Nano Banana Pro** | 1K / 2K / 4K | 10 standard | Yes | — |

**Nano Banana 2 Lite** is the fastest, lowest-cost option, built on Gemini 3.1 Flash Lite. It runs at a flat **$0.034 per image** (1K only), supports text-to-image and image-to-image with up to 10 reference images, and is ideal for rapid iteration and large batches such as Split Grid workflows.
...
The Split Grid node divides an image into a grid of cells and runs the **same node graph on every cell in parallel**. You design a per-cell template once, and Node Banana materializes real nodes on the canvas for each cell.
...
The Split Grid settings modal lets you pick any **rows × columns** layout. Common presets include 2×2 (4 cells), 2×3 and 3×2 (6 cells), 2×4 (8 cells), 3×3 (9 cells), and 2×5 (10 cells). Both 2×3 and 3×2 produce 6 cells but with different aspect ratios.

You can also drag the interior grid lines in the preview to create **non-uniform cells** — adjacent cells stay perfectly tiled with no seams or overlaps. Leaving the lines untouched keeps the cells evenly sized.

### Cell Templates

Each Split Grid node carries a **cell template** — a small node graph that is instantiated once per cell. Every template starts with a base **Cell Image** node that receives that cell's split image. You can add these node types to the template:

| Node | Purpose |
|------|---------|
| **Prompt** | Text prompt for the cell |
| **Generate Image** | AI image generation |
| **LLM Generate** | AI text generation |
| **Annotate** | Draw on the cell image |
| **Remove Background** | Strip the cell image background |
| **Resize Image** | Resize or re-encode the cell image |
| **Output** | Display the cell result |
| **Output Gallery** | Collect cell results |

### Shared Downstream Router

A template can wire one or more of its terminal outputs into a single shared **Router**, materialized once beside the whole grid (not per cell). Every cell's copy of those outputs connects into that Router, so all cell results converge onto one onward connection. The shared Router is preserved and re-wired when you resize the grid.

### Usage
1. Connect an image (like a contact sheet) to Split Grid
2. Choose a rows × columns layout, and optionally drag the grid lines for non-uniform cells
3. Build the cell template — for example, a Generate Image node wired to a Prompt for per-cell editing
4. Run the split; Node Banana instantiates the template for each cell and groups the nodes
5. Optionally route every cell's output through the shared Router into the rest of your workflow

<Callout type="info">
  Because each cell's nodes are real nodes on the canvas, cost estimates count them directly, so the on-canvas cost indicator reflects exactly what the grid will generate. Workflows saved before the template editor still load correctly.
</Callout>
