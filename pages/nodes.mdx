...
- **Optional/Required toggle** — Click the toggle in the node header to mark the node as Optional. Optional nodes without a loaded image are skipped during execution (shown with a dashed border and "Optional" drop-zone text). Downstream nodes that depend only on skipped nodes are also automatically skipped.
...
- Click to open file picker
- Supports MP3, WAV, OGG, FLAC, AAC, and other browser-supported audio formats
...
- **Optional/Required toggle** — Click the toggle in the node header to mark the node as Optional. Optional nodes without loaded audio are skipped during execution, and downstream nodes are skipped too.
...
- **Optional/Required toggle** — Click the toggle in the node header to mark the node as Optional. When Optional, the placeholder reads "Optional prompt (leave empty to skip)..." and an empty Prompt node is skipped during execution, with skip propagating downstream.
...
- **Lock/unlock** — Locked groups skip execution (lock icon shows in the title pill when locked)
- **NBP Input** — Mark a group as an NBP Input to visually distinguish it with a dashed border

### Group Context Menu

Group controls are accessed via the three-dot button next to the group title:

| Control | Description |
|---------|-------------|
| **Background** | Choose a color for the group background |
| **Lock / Unlock** | Toggle execution skip for the group |
| **NBP Input** | Toggle the dashed-border NBP Input visual style |
| **Delete** | Remove the group (nodes inside are kept) |

### Use Cases
- Organize related nodes visually
- Disable workflow sections without deleting
- Mark input groups for NBP workflows using the NBP Input toggle
