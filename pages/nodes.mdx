...
| [Generate Video](#generate-video) | AI video generation | Image, Video, Text, Audio | Video |
...
<Callout type="info">
  For Replicate, search covers the full catalogue — not just the most popular models. Type a name fragment to search Replicate's catalogue, or enter an exact `owner/name` id (e.g. `topazlabs/video-upscale`) to resolve a specific model directly even when it isn't in the popular list.
</Callout>
...
- **Video** (optional) — A source video for video-to-video models (upscaling, restoration, interpolation)
- **Text** — The prompt describing the video to generate
- **Audio** (optional) — An audio track for models that accept it
- **Dynamic inputs** — Additional inputs based on selected model's schema
...
<Callout type="info">
  You don't need to connect a prompt to run. Any one of a text, image, video, or audio input is enough — for example, a video-to-video upscaler runs with just a connected video.
</Callout>
...
Some providers offer models that transform an existing video rather than generating one from scratch — for example upscalers, restorers, and frame interpolators. To use one:

1. Connect a video source to the node's **Video** input handle.
2. Select a video-to-video model (these are classified as video models, so they appear in the model browser under video capabilities).
3. Run the workflow — no prompt is required.

<Callout type="info">
  Many video-processing models live outside the first pages of a provider's catalogue. Search by the exact model id (e.g. `topazlabs/video-upscale`) to pull it in directly. See [Model Discovery](#model-discovery).
</Callout>
