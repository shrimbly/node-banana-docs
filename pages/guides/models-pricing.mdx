...
Node Banana supports multiple AI providers for image, video, and text generation:

- **Gemini** — Google's image and video generation models
- **Replicate** — Wide selection of community and custom models
- **fal.ai** — Fast inference for popular models
- **Anthropic** — Claude models for text generation in LLM nodes
- **OpenAI** — GPT Image models for image generation, and GPT models for text generation in LLM nodes
...
OpenAI's GPT Image models are available for image generation once an `OPENAI_API_KEY` is configured. Both support text-to-image and image-to-image.

| Model | API Model | Capabilities | Cost |
|-------|-----------|--------------|------|
| **GPT Image 1** | gpt-image-1 | Text-to-image, image-to-image | ~$0.05 per image |
| **GPT Image 2** | gpt-image-2 | Text-to-image, image-to-image | ~$0.05 per image |
...
<Callout type="info">
  OpenAI image pricing varies by size and quality. The ~$0.05 per image shown in cost tracking is a flat estimate — check your [OpenAI usage dashboard](https://platform.openai.com/usage) for actual charges.
</Callout>

## Video Generation Models

Video generation is available through compatible models on Replicate, fal.ai, and Gemini providers.
...
Google Gemini Veo 3.1 models are available for video generation directly through the Gemini provider — no additional API key required beyond your existing Gemini API key.
...
- **Provider** — Compare pricing between Gemini, Replicate, and fal.ai
...
| gemini-3.1-pro | Slower | Best | Complex tasks |
| gemini-3-pro-preview | Slower | Best | Complex tasks |

### Anthropic Models

<Callout type="warning">
  Anthropic models require a separate `ANTHROPIC_API_KEY` environment variable.
</Callout>

| Model | Speed | Capability | Best For |
|-------|-------|------------|----------|
| claude-3-5-haiku | Fastest | Good | Quick prompts |
| claude-3-5-sonnet | Fast | Better | General use |
| claude-3-7-sonnet | Slower | Best | Complex tasks |
...
- Costs organized by provider (Gemini, Replicate, fal.ai, OpenAI)
