...
### Arrange Related Nodes
Keep logical sections visually together on the canvas:
- Input processing
- Generation pipeline
- Output handling

Select a set of nodes and use the alignment shortcuts (`H` to stack horizontally, `V` to stack vertically, `G` to arrange in a grid) to tidy them quickly.
...
### Pause Finished Branches
Click an edge to toggle its pause state and skip a completed branch. This:
