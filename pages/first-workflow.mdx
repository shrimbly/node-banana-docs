...
<Callout type="tip">
  You can also **double-click** (or **right-click**) an empty spot on the canvas to open a searchable node menu — type to find any node type, then press `Enter` to add it where you clicked.
</Callout>

## Step 2: Add a Generate Image Node

1. Click the **Generate** dropdown in the action bar (or press `Shift + G`)
2. Select **Image** from the dropdown menu (or choose **Video** for video generation, or **3D** for 3D model generation)
3. A Generate Image node appears on the canvas
4. Drag it to the right of your prompt node
...
- Click the **Run** button in the floating action bar to run all nodes
- Select specific nodes and choose **"Run N selected nodes"** from the Run dropdown to run only selected nodes
...
<Callout type="tip">
  The "Run selected nodes" feature lets you test specific parts of your workflow without running everything. Select the nodes you want to run, and they'll execute along with their dependencies.
</Callout>
