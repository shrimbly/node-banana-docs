...
- **Video → Video**: Video outputs connect to video inputs (in Output nodes)
...
- **Type safety**: You cannot connect incompatible types (e.g., text to image, 3D to image)
...
Handles are the connection points on nodes. Node Banana has four handle types:
