...
- **Floating action bar** — Use the buttons and dropdowns at the bottom of the screen
- **Keyboard shortcuts** — Press the shortcut for a node type (see below)
- **Connection drop menu** — Drag a connection from a node's handle into empty space to create and connect a compatible node in one step
- **Searchable node menu** — **Double-click** (or **right-click**) an empty spot on the canvas to open a searchable list of every node type

### Searchable Node Menu

The node search menu opens wherever you double-click or right-click on empty canvas and adds the chosen node at that position:

- Type to filter the full list of node types
- Use the **arrow keys** to move through results and **Enter** to add the highlighted node
- Press **Escape** or click away to dismiss the menu

import { Callout } from 'nextra/components'

<Callout type="tip">
  The searchable node menu is the fastest way to reach less-common nodes without hunting through the action bar — just double-click the canvas and start typing.
</Callout>
